...
public class CustomRun extends Thread {
	...
	public CustomRun(String runType, JTextArea showMsg, File[] files, String pathConfFile, String pathExportFile,
			String pathFreemarker, String entityPackageName,String language) {
	...
	public void run() {
		...
		if (runType.equals("client")) {
			...
			ExcelTo et = new ExcelToClient(pathConfFile, pathExportFile, pathFreemarker, showMsg, entityPackageName,language);
		...
		if (runType.equals("server")) {
			showMsg.append("\n\n生成服务器文件\n");
			ExcelTo et = new ExcelToServer(pathConfFile, pathExportFile, pathFreemarker, showMsg, entityPackageName,language);
			et.cleanGeneratedFile();
			et.processExcel(files);
		...
		if (runType.equals("phone")) {
			showMsg.append("\n\n生成手机端文件\n");
			ExcelTo et = new ExcelToPhone(pathConfFile, pathExportFile, pathFreemarker, showMsg, entityPackageName,language);
			et.cleanGeneratedFile();
			et.processExcel(files);
		}
	}
