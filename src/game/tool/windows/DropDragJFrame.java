	...
	public void drop(DropTargetDropEvent dtde) {
		...
		if (dtde.isDataFlavorSupported(DataFlavor.javaFileListFlavor)) {
			...
			try {
				...
				// 客户端
				CustomRun client = new CustomRun("client", showMsg, files, path + "/" + pathConf, path + "/"
						+ pathClient, path + "/" + pathFreemarker, entityPackage,"zhCN");
				client.run();

				// 服务器
				CustomRun server = new CustomRun("server", showMsg, files, path + "/" + pathConf, path + "/"
						+ pathServer, path + "/" + pathFreemarker, entityPackage,"zhCN");
				server.run();

				// 手机端
				CustomRun phone = new CustomRun("phone", showMsg, files, path + "/" + pathConf, path + "/" + pathPhone,
						path + "/" + pathFreemarker, entityPackage,"zhCN");
				phone.run();
